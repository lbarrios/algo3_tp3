...
for testname in tests:
  for testtype in tests[testname]:
    for testsize in tests[testname][testtype]:
      ...
      #y_p_values = [yp for yp in y_values if yp >= np.percentile(y_values,50) ]
...
colors = ['green','blue','red','cyan','magenta','yellow','black','grey','white']
...
plt.xlabel(u'Tamaño de entrada (n)')
...
for test_number in range(0,t_names):
  ...
  print x
  print y
  plt.plot(x, y, linestyle='-',  color=colors[test_number], linewidth=0.7, label=testtype, alpha=1) #, marker='.', markersize=0.3)
...
plt.savefig("../graficos/iteraciones_local_search_{}.pdf".format(datetime.now().strftime("%Y-%m-%d_%H-%M-%S")))
