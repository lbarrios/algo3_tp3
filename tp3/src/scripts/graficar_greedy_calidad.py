...
files = sorted(glob("../output/greedy_heuristic_A/*.txt.calidad"))
for f in files:
  file = open(f)
  filename = f.split("/")[-1]
...
files = sorted(glob("../output/greedy_heuristic_C/*.txt.calidad"))
for f in files:
  file = open(f)
  filename = f.split("/")[-1]
  testtype = filename.split("_")[0]
...
# Aplico formato
plt.grid(True)
plt.title(u"Heurísticas Golosas (densidad media)")
plt.ylabel('Omega 2')
...
plt.savefig("../graficos/calidad_greedy_{}.pdf".format(datetime.now().strftime("%Y-%m-%d_%H-%M-%S")))
